...
print('STAN_Space-time Memory Networks: initialized.')
...
class Encoder_M(nn.Module):
    ...
    def forward(self, in_f, in_m, in_o):
        #in_f=in_f.detach().cpu()
        mean_cpu=self.mean.detach().cpu()
        std_cpu=self.std.detach().cpu()
                    ###########################################################
        f = (in_f - mean_cpu) / std_cpu
...
class Decoder(nn.Module):
    ...
    def forward(self, r4, r3, r2):
        ...
        return m4,m3,m2,p #, p2, p3, p4        #p class x h x w
...
class STM(nn.Module):
    ...
    def segment(self, frame, keys, values, num_objects): 
        ...
        feats = self.Decoder(m4, r3e, r2e)
        ps = F.softmax(feats[-1], dim=1)[:,1] # no, h, w  
